...
def get_optimal_team(pokemon_list, gen_num=8):
    if len(pokemon_list) <= 6:
        ...
        for all_team_types in tqdm(possible_team_types):
            ...
            attack_types = reduce(lambda x, y: x + y, [[team_type] if isinstance(team_type, str) else list(team_type) for team_type in all_team_types])
...
def main():
    ...
    chart = get_gen_type_chart(4)
    print_chart = '\n'.join([' | '.join([str(x) + ''.join([' ' for _ in range(4 - len(str(x)))]) for x in chart[i]]) for i in range(len(chart))])
    print(f"<p>{print_chart}</p>")
